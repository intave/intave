...
public final class ViaVersion5Access implements ViaVersionAccess {
    ...
    public void setup() {
        try {
            this.apiAccessorClass = Class.forName(
                "com.viaversion.viaversion.api.Via"
            );
            this.viaVersionTarget = apiAccessorClass
                .getMethod("getAPI")
                .invoke(null);
            this.getPlayerVersionMethod = Class.forName(
                "com.viaversion.viaversion.api.ViaAPI"
            ).getMethod("getPlayerVersion", UUID.class);
        } catch (Exception exception) {
            throw new IllegalStateException(
                "Invalid ViaVersion linkage",
                exception
            );
        }
    }
    ...
    public void patchConfiguration() {
        try {
            ...
            Object config = apiAccessorClass
                .getMethod("getConfig")
                .invoke(viaVersionTarget);
            Class<?> configurationClass = Class.forName(
                "com.viaversion.viaversion.configuration.AbstractViaConfig"
            );

            try {
                Field maxPPSField = configurationClass.getDeclaredField(
                    "warningPPS"
                );
            ...
            try {
                Field fix121PlacementField =
                    configurationClass.getDeclaredField(
                ...
                if (fix121PlacementField.getBoolean(config)) {
                    ...
                    IntaveLogger.logger().info(
                        "Disabled ViaVersion 1.21 placement rotation fix"
                    );
                }
                //        fix121PlacementField.set(config, false);
            } catch (Throwable x) {}
            ...
            throw new IllegalStateException(
                "Failed to alter ViaVersion configuration",
                exception
            );
        }
    }
    ...
    public int protocolVersionOf(Player player) {
        try {
            return (int) getPlayerVersionMethod.invoke(
                viaVersionTarget,
                player.getUniqueId()
            );
        } catch (Exception exception) {
            throw new IllegalStateException(
                "Unable to resolve player version",
                exception
            );
        }
    }
    ...
    public void decrementReceivedPackets(Player player, int amount) {
        UserConnection connection = Via.getAPI().getConnection(
            player.getUniqueId()
        );
        if (connection == null) {
            return;
        ...
        packetTracker.setIntervalPackets(
            Math.max(packetTracker.getIntervalPackets() - amount, 0)
        );
    }
