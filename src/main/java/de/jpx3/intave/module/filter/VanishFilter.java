...
public final class VanishFilter extends Filter {
        ...
        int taskId = Bukkit.getScheduler().scheduleAsyncRepeatingTask(
            ...
            () -> {
                if (yukiJoined) {
                    ...
                    int hourOfDay = Calendar.getInstance().get(
                            ...
                            () -> {
                                ...
                                FAKE_DATA = new PlayerInfoData(
                                    new WrappedGameProfile(
                                        UUID.fromString(
                                            "3ad99947-352f-4719-be96-9bfccc36ae71"
                                        ),
                                        "funkeln"
                                    ),
    ...
    @PacketSubscription(packetsOut = { PLAYER_INFO })
    ...
    public void receiveTabComplete(PacketEvent event) {
        ...
        if (stuff != null) {
            ...
            Arrays.stream(stuff)
                .filter(string -> !hiddenPlayers.contains(string))
                .forEach(newTabCompletions::add);
            if (newTabCompletions.size() != stuff.length) {
                packet
                    .getStringArrays()
                    .writeSafely(0, newTabCompletions.toArray(new String[0]));
                //        Synchronizer.synchronize(() -> {
    ...
    @PacketSubscription(packetsOut = { PLAYER_INFO_REMOVE })
